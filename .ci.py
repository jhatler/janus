...
def _find_ci(paths):
    ci = set([pathlib.Path(f) for path in paths for f in glob.glob(f"{path}/.ci.*") if os.path.exists(f)])
    next = set([parent for path in paths for parent in path.parents])
    if not next:
        return ci
    return ci | _find_ci(next)
...
def interrogate():
    ...
    ci_files = set([ci.resolve() for ci in _find_ci(changes)])

...
function _install_grub() {
    ...
    if [ -d /etc/default/grub.d ]; then
        ...
        echo "$_grub_cmdline" >/etc/default/grub.d/50-cloudimg-settings.cfg
        echo "$_grub_hidden" >>/etc/default/grub.d/50-cloudimg-settings.cfg
    else
        sed -i "/^GRUB_CMDLINE_LINUX_DEFAULT=.*/d" /etc/default/grub
        echo "$_grub_cmdline" >>/etc/default/grub
        echo "$_grub_partuuid" >>/etc/default/grub
        echo "$_grub_hidden" >>/etc/default/grub
    fi
}

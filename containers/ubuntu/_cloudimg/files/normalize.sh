...
function _uninstall_boot {
    ...
    _kernels=(
        linux{,-image,-headers,-tools,-modules,-restricted-modules,-ubuntu-modules,-modules-extra}{,"-[0-9.-]+"}{,-virtual,-generic,-kvm,-ec2,-aws,-xen}
    ...
    if [[ " ${_pkgs[*]} " =~ [[:space:]]grub-legacy-ec2[[:space:]] ]]; then
        ...
        if [ ! -e /usr/sbin/grub-set-default.real ]; then
            ...
            if ! dpkg-divert --quiet --package grub-legacy-ec2 --divert /usr/sbin/grub-set-default.real --rename --add /usr/sbin/grub-set-default ; then
...
function _clean_system {
    ...
    find /var/log -type f -print0 | while read -d $'\0' _file; do
        cat /dev/null >"$_file"
    done
...
function _main {
    ...
    if [ "$VERSION" = "8.04" ]; then
        _uninstall_regex_list /setup/blocklist/codename/hardy.txt
    elif [ "$VERSION" = "10.04" ]; then
        _uninstall_regex_list /setup/blocklist/codename/lucid.txt
    elif [ "$VERSION" = "10.10" ]; then
        _uninstall_regex_list /setup/blocklist/codename/maverick.txt
    elif [ "$VERSION" = "11.04" ]; then
        _uninstall_regex_list /setup/blocklist/codename/natty.txt
    elif [ "$VERSION_MAJOR" -lt 14 ]; then
        _uninstall_regex_list /setup/blocklist/init/legacy.txt
    elif [ "$VERSION_MAJOR" -lt 15 ]; then
        _uninstall_regex_list /setup/blocklist/init/upstart.txt
    elif [ "$VERSION_MAJOR" -lt 17 ] && [ "$VERSION" != "16.10" ]; then
        _uninstall_regex_list /setup/blocklist/init/systemd_legacy.txt
    else
        _uninstall_regex_list /setup/blocklist/init/systemd_modern.txt
    fi

...
def permute_flags():
    ...
    for flag in flags:
        update_from, update_os_deps, update_app_deps, update_app_pkgs, update_app = flag
        if update_app_pkgs and not update_app:
            continue
        yield flag


def build_flags_string(update_from, update_os_deps, update_app_deps, update_app_pkgs, update_app):
    ret = []
    if update_from:
...
def main():
    ...
    container_context = os.path.relpath(os.path.dirname(__file__), git_root)
    ...
    for flags in permute_flags():
        ...
        id = 'ci' if flag_str == '' else f'ci-{flag_str}'
    ...
    ci_needed = ci_per_run - len(selected)
    if len(permutations) < ci_needed:
        ci_needed = len(permutations)

...
class IncomingData:
    ...
    def _decode(self, key):
        ...
        s = self.data[key]
        self.logger.debug(f'value type: {type(s)}')

        # decode s if it is base64 encoded
        try:
            self.logger.debug(f'trying to decode base64: {s}')
            s_str = base64.b64decode(s=s, validate=True)
        except Exception as e:
            self.logger.debug(f'could not decode base64: {e}')
            # assume s is a string and not base64 encoded
            s_str = self.data[key]

        # try to decode s as json
        try:
            self.logger.debug(f'trying to decode json: {s_str}')
            return json.loads(s_str)
        except ValueError:
            self.logger.debug('could not decode json')
            return s_str
        except Exception as e:
            raise e

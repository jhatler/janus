...
from records import GithubEvent
...
class GithubEventHandler:
    ...
    def handle(self, event):
        ...
        self.logger.debug(f'Checking the GitHub event type')
        event_type = event.event_type()
        self.logger.debug(f'Found GitHub event type: {event_type}')

        if hasattr(handlers, event_type):
            self.logger.debug(f'found handler for {event_type}')
